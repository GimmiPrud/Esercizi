...
un dizionario (rented_film) che ha come chiave un numero intero rappresentante l'id del cliente che ha affittato il film e
per valore una lista contenente i film affittati dal cliente.
...
- Definire i seguenti metodi:
    ...
    7 il metodo deve calcolare la penale che il cliente deve pagare utilizzando l'opposito metodo.
    Stampare la penale che il cliente deve pagare: "Cliente: {clientID}! La penale da pagare per il film {titolo_film} e' di {tot} euro!".
    printMovies(): stampa la lista di tutti i film disponibili in negozio. 
...
from movie_genre import Commedia,Drama,Azione
class Noleggio:

    def __init__(self, film_list: list[Film], rented_film: dict):
        self.film_list = film_list
        self.rented_film = rented_film
